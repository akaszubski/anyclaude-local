#!/usr/bin/env python3
"""
Analyze vLLM-MLX trace files to benchmark performance and cache effectiveness.
...
class TraceAnalyzer:
    def __init__(self, trace_dir=None):
        if trace_dir is None:
            trace_dir = Path.home() / ".anyclaude" / "traces" / "vllm-mlx"
    ...
    def load_traces(self):
        ...
        print(f"📁 Found {len(trace_files)} trace files in {self.trace_dir}\n")
    ...
    def analyze_cache_effectiveness(self):
        ...
        for req_hash, group in request_groups.items():
            if len(group) == 1:
                ...
                print(f"🎯 Request appears {len(group)} times:")
                ...
                for j, req in enumerate(group):
                    ...
                    if j == 0:
                        ...
                        print(f"        ↳ This is a cache hit candidate (duplicate request)")
    ...
    def analyze_timing(self):
        ...
        print()
        print("💡 To measure cache effectiveness:")
    ...
    def generate_benchmark_commands(self):
        ...
        print("  3. If 2nd request is much faster → cache is working")
        print()

        print("Active benchmarking (send test requests):")
        print("  # Start server with debug logging")
        print("  PROXY_ONLY=true ANYCLAUDE_DEBUG=1 anyclaude --mode=vllm-mlx &")
        print()
        ...
        print("  # Check vLLM-MLX server output for timing info")
