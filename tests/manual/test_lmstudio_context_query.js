...
http.get('http://localhost:1234/api/v0/models', (res) => {
  let data = '';
  ...
  res.on('end', () => {
    try {
      ...
      const expectedContext = loadedModel.loaded_context_length || loadedModel.max_context_length;
      console.log(`   ✅ LMStudio loaded model: ${loadedModel.id}`);
      console.log(`   ✅ Context length: ${expectedContext.toLocaleString()} tokens`);
      console.log('');
          ...
          res.on('end', () => {
            ...
            setTimeout(() => {
              ...
              if (!contextDetected) {
                ...
                console.log('   Expected to see: [Context] Using LMStudio reported context');
                process.exit(1);
              }

              console.log(`✅ Context length detected: ${detectedContext.toLocaleString()} tokens`);
              console.log(`✅ LMStudio reported:      ${expectedContext.toLocaleString()} tokens`);
              console.log('');

              if (detectedContext === expectedContext) {
                console.log('✅ PERFECT MATCH! Context length correctly queried from LMStudio');
                console.log('');
                process.exit(0);
              } else {
                console.log(`⚠️  MISMATCH! Expected ${expectedContext} but got ${detectedContext}`);
                console.log('');
                process.exit(1);
              }
            }, 1000);
      ...
      console.log(`   ❌ Failed to parse LMStudio response: ${error.message}`);
      process.exit(1);
    }
  });
}).on('error', (err) => {
