...
async function testProxy() {
  ...
  try {
    ...
    while (true) {
      ...
      console.log(
        `[Raw chunk ${rawChunks}] ${value.length} bytes: ${text.substring(0, 100)}${text.length > 100 ? "..." : ""}`
      );
      ...
      for (const line of lines) {
        if (line.startsWith("data: ")) {
          ...
          try {
            ...
            console.log(
              `  → [SSE Chunk ${chunks}] type="${json.type}"${detail}`
            );
          } catch (e) {
            console.log(
              `  → [SSE Chunk ${chunks}] (parse error: ${e.message})`
            );
          }
        }
      }
