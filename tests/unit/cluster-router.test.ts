 ...
 */
function createTestNode(id: string, overrides?: Partial<MLXNode>): MLXNode {
  return {
    id: id as NodeId,
    ...
    test("should add +50 for cache match", () => {
      ...
      const nodes = [
        ...
        createTestNode("node-2", {
          cache: {
            tokens: 1000,
            systemPromptHash: "hash-different",
            lastUpdated: Date.now(),
          },
        }),
      ];
...
describe("ClusterRouter", () => {
  ...
  describe("selectNodeWithSticky", () => {
    ...
    test("should create new session when session not found", () => {
      ...
      const decision = router.selectNodeWithSticky(
        nodes,
        context,
        "new-session"
      );
    ...
    test("should update session when node changes", () => {
      ...
      const decision2 = router.selectNodeWithSticky(
        nodes,
        context,
        "session-1"
      );
