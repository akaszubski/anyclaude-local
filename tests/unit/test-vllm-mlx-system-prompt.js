...
function test_complex_whitespace_normalization() {
  ...
  assert.ok(!content.includes("\t"), "No tabs should remain");
  assert.ok(!/  /.test(content), "No double spaces should remain");
...
function test_normalized_json_is_valid() {
  ...
  let error = null;
  try {
    const normalized = interceptor.interceptAndNormalize(
