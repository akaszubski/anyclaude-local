...
The issue was in how `anyclaude` spawns the vLLM-MLX server:

1. **Auto-launch behavior**: The code was configured to automatically spawn `scripts/vllm-mlx-server.py` as a child subprocess when `anyclaude` starts
...
The vLLM-MLX server itself **works fine** when started directly in a terminal, but fails when spawned as a subprocess.
...
"vllm-mlx": {
  ...
  "description": "vLLM-MLX - start manually to avoid crash dialogs"
}
```
...
- **Logging to file**: All server output is written to `~/.anyclaude/logs/vllm-mlx-server.log`
...
If the vLLM-MLX server fails to start, check the logs:
...
- `.anyclauderc.json`: Set vLLM-MLX model to "current-model" to disable auto-launch
